...
ccys = ['EURUSD', 'EURCHF', 'EURGBP', 'EURAUD']


class strat_reversal(object):
    def __init__(self, w_dir,directory_load,directory_save,ccy,year,month,params):
        
        self.df = dd.read_hdf(directory_load+'/'+ccy+'-'+str(year)+'.h5',ccy+str(year)+str(month).zfill(2))
        ...
        if month ==1:
            ...
            prev_y = str(year)
            prev_m = str(month-1).zfill(2)
            
        self.train_df = dd.read_hdf(directory_load+'/'+ccy+'-'+prev_y+'.h5', ccy+prev_y+prev_m)
        self.w_dir = w_dir
    ...
    def execute(self,df,params):
        ...
        if self.save_CF:
            df['CF'].to_hdf(directory_save+'/Strat_Reversal'+ccy+str(self.year)+str(self.month).zfill(2)+'.h5','Strat_Reversal',format='table')#, ccy+year+month.zfill(2)
            with open(directory_save+'/Report.txt', 'a') as fout:
                fout.write('\n'.join([
                        '\n\n=======================================================',
                        '            Ccy: '+self.ccy+'          Month: '+str(self.month),
...
for ccy in ccys:
    ...
    for month in range(1,12):
        ...
        strat = strat_reversal(w_dir,directory_load,directory_save,ccy,year,month,params)
        strat.train(20)
        strat.save_CF = True
